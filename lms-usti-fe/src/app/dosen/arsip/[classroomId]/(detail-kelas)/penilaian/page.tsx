...
async function ArchivedClassroomGradesContent({ classroomId }: { classroomId: string }) {
...
export default async function ArchivedPenilaianPage({
  params,
}: {
  ...
  return (
    <Suspense fallback={<ClassroomGradesSkeleton />}>
      <ArchivedClassroomGradesContent classroomId={classroomId} />
    </Suspense>
  );
}
